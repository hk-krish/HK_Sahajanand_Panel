...
const ImageUpload: FC<ImageUploadProps> = ({ fileList, setFileList, multiple, setValue, name, trigger, accept, isListType }) => {
  ...
  const customUpload: UploadProps["beforeUpload"] = async (file) => {
    ...
    try {
      ...
      uploadedPhotoURLs.push(response.data as string);
      const updatedList: any = multiple ? [...fileList, ...uploadedPhotoURLs] : uploadedPhotoURLs;
      setFileList(multiple ? updatedList : [updatedList[0]]);
      if (name) {
